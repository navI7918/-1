...
// Проверяем наличие Lampa перед запуском
...
if (typeof Lampa.Component === 'undefined' || typeof Lampa.SettingsApi === 'undefined') {
    console.error('Lampa components not fully loaded');
    return;
}
...
// Проверяем наличие Lampa.PlayerPlaylist перед использованием
if (typeof Lampa.PlayerPlaylist !== 'undefined' && Lampa.PlayerPlaylist.listener) {
    Lampa.PlayerPlaylist.listener.follow('select', function(e) {
        if (e.item.plugin && e.item.plugin === plugin.component && Lampa.Player && Lampa.Player.runas)
            Lampa.Player.runas(Lampa.Storage.field('player_iptv'));
    });
}

function channelSwitch(dig, isChNum) {
	if (!Lampa.Player || !Lampa.Player.opened()) return false;
	...
	if (number && number <= cnt) {
		...
		var chSwitch = function () {
			...
			if (Lampa.PlayerPlaylist.position() !== pos) {
				...
				if (Lampa.Player && Lampa.Player.runas) Lampa.Player.runas(Lampa.Storage.field('player_iptv'));
			}
...
function keydown(e) {
	var code = e.code;
	if (Lampa.Activity && Lampa.Activity.active() && Lampa.Activity.active().component === plugin.component
		&& Lampa.Player && Lampa.Player.opened()
...
// Добавляем стили только если Lampa.Template существует
			...
			var parseList = function () {
				...
				while (i < l.length) {
					...
					if (!!channel.Url && !channel.isYouTube) {
						// --- ФИЛЬТР 18+ ---
						var isAdult = false;
						var adultMarkers = ['18+', '+18', 'adult', 'xxx', 'porn', 'sex', 'erot'];
						var titleCheck = channel.Title.toLowerCase();
						var groupCheck = channel.Group.toLowerCase();
						var tvgNameCheck = (channel['tvg-name'] || '').toLowerCase();
						
						for (var markerIdx = 0; markerIdx < adultMarkers.length; markerIdx++) {
							var marker = adultMarkers[markerIdx];
							if (titleCheck.includes(marker) || 
								groupCheck.includes(marker) || 
								tvgNameCheck.includes(marker)) {
								isAdult = true;
								break;
							}
						}
						
						var adultFilterEnabled = getSettings('adult_filter') === 'true';
		...
		Lampa.Select.show({
			...
			onSelect: function(group) {
				if (object.currentGroup !== group.key) {
					...
					if (Lampa.Player) Lampa.Player.opened() || Lampa.Controller.toggle('content');
				}
			},
			onBack: function() {
				if (Lampa.Player) Lampa.Player.opened() || Lampa.Controller.toggle('content');
...
// --- НАСТРОЙКА ФИЛЬТРА 18+ ---
langAdd('adult_filter', {
    ru: 'Фильтр 18+',
    uk: 'Фільтр 18+',
    be: 'Фільтр 18+',
    en: 'Adult filter',
...
langAdd('adult_filter_desc', {
    ru: 'Скрыть каналы с пометкой 18+',
    uk: 'Приховати канали з позначкою 18+',
    be: 'Схаваць каналы з пазнакай 18+',
    en: 'Hide channels marked 18+',
    zh: '隐藏标记为 18+ 的频道'
});
...
addSettings(
	'trigger',
	{
		title: langGet('adult_filter'),
		description: langGet('adult_filter_desc'),
		name: 'adult_filter',
		default: false,
		onChange: function(v){
			var currentActivity = Lampa.Activity.active();
			if (currentActivity && currentActivity.component === plugin.component) {
				catalog = {};
				curListId = -1;
				Lampa.Activity.replace(Lampa.Arrays.clone(currentActivity.activity));
			}
		}
	}
);
...
if (!!window.appready) pluginStart();
    ...
    if (typeof Lampa.Listener !== 'undefined') {
        Lampa.Listener.follow('app', function(e){if (e.type === 'ready') pluginStart()});
    } else {
        // Если Lampa.Listener недоступен, запускаем сразу
        setTimeout(pluginStart, 1000);
    }
}
})();
